...
export const jwtConfig = registerAs('JWT_CONFIG', () => ({
  ...
  accessTokenExpiresIn: (process.env.JWT_ACCESS_EXPIRES_IN || '1h') as ms.StringValue,
  refreshToken: process.env.JWT_REFRESH_KEY || 'jwtrefreshkey',
  refreshTokenExpiresIn: (process.env.JWT_REFRESH_EXPIRES_IN || '7d') as ms.StringValue,
}));
